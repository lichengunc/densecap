...
- roi_features: B2 x D x HH x WW array of features for RoIs sampled as positives 
                and negatives, to be sliced using PosSlicer in DenseCapModel.
...
function build_rpn(opt)
  ...
  box_branch:add(box_conv_layer) -- transforms N x 4k x H x W
  box_branch:add(nn.RegularizeLayer(opt.box_reg_decay))
  local x0, y0, sx, sy = unpack(opt.field_centers)
  local seq = nn.Sequential()
  seq:add(nn.MakeAnchors(x0, y0, sx, sy, anchors))  -- anchors N x 4k x H x W
  seq:add(nn.ReshapeBoxFeatures(num_anchors))  -- anchors N x (k * H * W) x 4
  local cat1 = nn.ConcatTable()
  cat1:add(seq) -- anchors N x (k * H * W) x 4
  cat1:add(nn.ReshapeBoxFeatures(num_anchors)) -- transforms N x (k * H * W) x 4
  box_branch:add(cat1)  -- {anchors (N, kHW, 4), transforms (N, kHW, 4)}
  local cat2 = nn.ConcatTable()
  cat2:add(nn.ApplyBoxTransform()) -- boxes (N, kHW, 4)
  cat2:add(nn.Identity())  -- {anchors (N, kHW, 4), transforms (N, kHW, 4)}
  box_branch:add(cat2)  -- {boxes, {anchors, transforms}}
  ...
  rpn_branch:add(nn.ReshapeBoxFeatures(num_anchors)) -- score (N, kHW, 2)
  ...
  rpn:add(concat)
  rpn:add(nn.FlattenTable()) -- {boxes(N, kHW, 4), anchors (N, kHW, 4), transforms(N, kHW, 4), score (N, kHW, 2)}
